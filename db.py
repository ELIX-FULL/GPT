...
load_dotenv()
pool = AsyncConnectionPool(conninfo=getenv("DATABASE_URL"), timeout=10, max_lifetime=600,
...
class DataBase:
    ...
    async def new_order(invoice_id: int, user_id: int, product: str):
        async with pool.connection() as conn:
            async with conn.cursor() as cursor:
                await cursor.execute("INSERT INTO orders(invoice_id, user_id, product) VALUES (%s, %s, %s)",
                                     (invoice_id, user_id, product))
                await conn.commit()
